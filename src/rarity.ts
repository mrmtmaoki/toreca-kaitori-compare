...
const ONE_PIECE_TABLE: Record<string, string> = {
  ...
  // 遊々亭は「P-」接頭辞でパラレルを表す(例: "P-SEC")一方、name側にも重複して
  // "(パラレル)"タグを付ける(例: "エネル(パラレル)")。もしこれを他店同様
  // "シークレットレア(パラレル)"に寄せると、name側のタグが作るvariant="パラレル"
  // と二重にパラレルを表現してしまい、rarity="シークレットレア(パラレル)"+
  // variant="パラレル"という他店には無い組み合わせになって突合できなくなる。
  // 代わりに基本ランクへ寄せ、パラレルの表現はvariant側だけに任せる
  // (normalizeParallelEncodingを経由せずとも他店のrarity="リーダー"+
  // variant="パラレル"と同じ最終形に揃う)。
  "P-C": "コモン",
  "P-UC": "アンコモン",
  "P-R": "レア",
  "P-SR": "スーパーレア",
